...
public class RecentPostsRule implements OrderRule {
  ...
  public List<JobOffer> apply(List<JobOffer> offers) {
    ...
    return offers.stream()
        .sorted(Comparator.comparing((JobOffer offer) ->
            offer.createdAt().isAfter(now.minusDays(DAYS_THRESHOLD))
                ? 0 : 1 // recientes primero
        ).thenComparing(JobOffer::createdAt).reversed())
        .toList();
  }
}
